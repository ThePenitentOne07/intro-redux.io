...
const RootComponent = (props) => {
  ...
  const addProductToCart = (product) => {
    setCart((prevCart) => {
      const productInCart = prevCart.products.find(p => p.id === product.id);
      let newProducts;

      if (productInCart) {
        newProducts = prevCart.products.map(p =>
          p.id === product.id ? { ...p, qty: p.qty + 1, price: product.price * (p.qty + 1) } : p
        );
      } else {
        newProducts = [...prevCart.products, { ...product, qty: 1 }];
      }

      const newTotalPrice = newProducts.reduce((total, p) => total + p.price, 0);
  ...
  const removeProductFromCart = (product) => {
    setCart((prevCart) => {
      ...
      if (productInCart && productInCart.qty > 1) {
        ...
        newProducts = prevCart.products.map(p =>
          p.id === product.id ? { ...p, qty: 0, price: 0 } : p
        );
      }
